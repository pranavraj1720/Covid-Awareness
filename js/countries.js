...
// SHOW/hidden THE COUTRY LIST ON CLICK EVENT
chang_country_btn.addEventListener("click", function(){
    input.value = "";
    resetCountryList();
    search_country_element.classList.toggle("hidden");
...
input.addEventListener("input", function(){
    ...
    country_list.forEach( country => {
        if( country.name.toUpperCase().startsWith(value)){
            document.getElementById(country.name).classList.remove("hidden");
        }else{
            document.getElementById(country.name).classList.add("hidden");
        }
    })
})
